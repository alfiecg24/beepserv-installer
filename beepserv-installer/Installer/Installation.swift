...
func doDirectInstall(_ device: Device) async -> Bool {
    ...
    if !persistenceHelperCandidates[0].isInstalled {
        Logger.log("Tips is not installed, please install it", type: .error)
        if !supportsFullPhysRW {
...
func doIndirectInstall(_ device: Device) async -> Bool {
    ...
    var success = false
    if !install_persistence_helper_via_vnode(candidates[0].bundlePath) {
        Logger.log("Failed to install persistence helper to Tips", type: .error)
    } else {
        Logger.log("Successfully installed persistence helper to Tips!", type: .success)
        success = true
    }
